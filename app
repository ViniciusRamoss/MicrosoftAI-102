...
def configure_interface():
    st.title("Upload de arquivos dio desafio 1 python Azure Fake docs")
    ...
    if uploaded_file is not None:
        ...
        if blob_url:
            st.write(f"Arquivo {fileName} enviado com sucesso para o Azure Blobo Storage")
            credit_card_info = analize_credit_card #chamar função de cartão de crédito
...
def show_image_and_validation(blob_url, credit_card_info):
    ...
    if credit_card_info and credit_card_info["card_name"]:
        ...
        st.write("Este não é um cartão de crédito valido")
